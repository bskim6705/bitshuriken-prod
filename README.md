...
Images are built by each app repo's GitHub Actions and pushed to **GHCR**; this
...
Deployed services: **be** (spot/futures/portal, 3 processes from 1 image),
...
### 2. Clone + env
```bash
sudo git clone https://github.com/bskim6705/bitshuriken-prod-infra /opt/bitshuriken-prod-infra
cd /opt/bitshuriken-prod-infra
...
Each push to an app repo's `main` branch builds a fresh `:latest` image (GitHub
Actions → GHCR). To roll it out, the host **pulls + runs** — that's the whole flow:
...
- Order-book liquidity is not part of this stack — without an external maker feeding orders, books are empty. (The legacy mirroring bots live in the archived `bitshuriken-v2` workspace and can be pointed at this deployment later if needed.)
