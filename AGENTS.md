...
spot·futures 완성도에 집중하는 시뮬레이션 거래소. `bitshuriken-v2`의 시맨틱 포크 — 코어 런타임에서 options/dex 제외(mcp는 [ADR-074](docs/adr/074-prod-docs-mcp-server.md)로 개발자 도구 복귀), bots/agents는 외부 전략 테스트 서비스로 포함 (경위: [docs/adr/065](docs/adr/065-production-fork.md) 포크 → ADR-066 재도입). "prod는 실거래소 기준으로 판단한다" (docs/feedback/024).
...
| repo | 역할 | port |
| --- | --- | --- |
| [bitshuriken-prod-be](bitshuriken-prod-be/) | Nest.js — apps/{spot, futures, portal, settle} + libs | 5101 / 5102 / 5103 / 5104(settle 헬스) |
...
| [bitshuriken-prod-mcp](bitshuriken-prod-mcp/) | API 레퍼런스 MCP 서버 (읽기 전용) | — (stdio) |
| [bitshuriken-prod-fly](bitshuriken-prod-fly/) | 실험: FlyWire 초파리 커넥톰 트레이딩 뇌 (spot 롱온리) | 5130 |
...
## Docs (이 우산에 위치)
- [docs/adr/](docs/adr/) — 아키텍처 의사결정. 새 결정 발생 시 `docs/adr/NNN-제목.md` (Status/Context/Decision/Rationale/Consequences). 번호 순차 증가, 다음은 079. 코드 주석에 ADR 번호 박지 않는다.
...
- 제품 단계는 spot·portal GA / futures beta (ADR-076). beta 장애가 GA로 번지는 결합을 만들지 않는다 — 자기 프로세스·토픽·컨슈머 그룹, 의존 방향은 beta→GA만, nginx 기동 의존·공유 컨슈머 금지. 격리 매트릭스와 GA 종료 조건은 ADR-076 §5·§6.
...
Windows는 WSL2 + Docker Desktop WSL integration을 사용한다. 개별 서비스 실행/컨벤션은 각 디렉터리의 AGENTS.md 참조(CLAUDE.md는 그 심볼릭 링크).
