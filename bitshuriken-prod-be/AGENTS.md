...
- `apps/settle` (port 5104, `/health` 전용) — M1 정산 프로세스(ADR-077): `match.*.out` DB 효과 컨슈머(그룹 `bitshuriken-settle`) + spot·futures 정산 워커 + DLQ + 원장 읽기 레플리카·프로젝터·드리프트. API 앱의 out 컨슈머는 인메모리 효과만(DB 무접촉)
