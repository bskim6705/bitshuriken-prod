/** Minimal timestamped logger with per-scope tags. Logs to stderr so an stdio MCP can share the module. */
const C = {
  dim: '\x1b[90m',
  red: '\x1b[31m',
  green: '\x1b[32m',
...
export function makeLogger(scope: string) {
  ...
  // everything to stderr: a stdio MCP transport owns stdout.
  return {
