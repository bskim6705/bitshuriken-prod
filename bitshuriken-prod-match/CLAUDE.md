...
- `schemas/` — 메시지 스키마.
- `config/` — `tickers.json`(전체), `tickers-spot.json`, `tickers-futures.json`. 엔진은 config에서 lane을 생성(마켓은 env `MATCH_CONFIG_PATH`로 선택).
...
- stepSize/tickSize = `10^(8 - precision)` int, OrderBook이 보유. (전체 표는 `../CLAUDE.md`.)
